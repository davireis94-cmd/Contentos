...
// Curated dark tech/business Unsplash photos
const DARK_PHOTOS = [
...
function extractLayout(body: string): string {
...
function swipeArrow(dark: boolean): string {
  ...
  return `
    <div class="swipe-arrow" style="background:${bg};">
...
function buildSlide(slide: Slide, total: number, photoIdx: number): string {
  const layout = extractLayout(slide.body);
  const cleanBody = stripNotes(slide.body);
  const isLast = slide.index === total - 1;
  const prog = progressBar(slide.index, total, layout !== "light" && layout !== "feature-list" && layout !== "step-list");

  switch (layout) {
    case "dark-photo": {
      const photo = getPhoto(photoIdx);
      return `
      <div class="slide dark" id="slide-${slide.index}">
        <div class="img-bg">
          <img src="${photo}" alt="" style="width:100%;height:100%;object-fit:cover;" crossorigin="anonymous">
        </div>
    ...
    case "gradient": {
      return `
      <div class="slide gradient" id="slide-${slide.index}">
        <div class="slide-content center">
          ...
          ${slide.cta ? `<div class="cta-btn">${escapeHtml(slide.cta)}</div>` : '<div class="cta-btn">Seguir @davimoxoto</div>'}
        </div>
        ${prog}
      </div>`;
    }
    ...
    case "light": {
      return `
      <div class="slide light" id="slide-${slide.index}">
    ...
    case "step-list": {
      ...
      const rows = steps.length > 0
        ? steps.map((s) => `
          <div class="step-row" style="border-bottom:1px solid var(--light-border);">
    ...
    default: { // dark
      const photo = layout === "dark" ? null : getPhoto(photoIdx);
      return `
      <div class="slide dark" id="slide-${slide.index}">
        ${photo ? `<div class="img-bg"><img src="${photo}" alt="" style="width:100%;height:100%;object-fit:cover;" crossorigin="anonymous"></div><div class="img-overlay" style="background:linear-gradient(to top,#180E0C 45%,rgba(24,14,12,0.75) 70%,rgba(24,14,12,0.3) 100%);"></div>` : ""}
        <div class="slide-content">
...
function generateHtml(slides: Slide[], title: string): string {
  const total = slides.length;
  let photoCounter = 0;
  const slidesHtml = slides
    .map((s) => {
      const layout = extractLayout(s.body);
      const html = buildSlide(s, total, layout === "dark-photo" ? photoCounter : photoCounter);
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .ig-avatar { width:36px; height:36px; border-radius:50%; background:var(--brand-primary); display:flex; align-items:center; justify-content:center; font-family:'Playfair Display',serif; color:#fff; font-size:14px; font-weight:600; flex-shrink:0; position:relative; overflow:hidden; }
  .ig-handle { font-size:13px; font-weight:600; color:#262626; }
  .ig-subtitle { font-size:11px; color:#8e8e8e; }
  .ig-dots-btn { margin-left:auto; font-size:18px; color:#262626; cursor:pointer; }
  .carousel-viewport { width:420px; aspect-ratio:4/5; overflow:hidden; position:relative; cursor:grab; }
  .carousel-viewport:active { cursor:grabbing; }
  .carousel-track { display:flex; width:calc(420px * ${total}); height:100%; transition:transform 0.35s cubic-bezier(0.25,0.46,0.45,0.94); }
  ...
  .slide.gradient { background:var(--gradient); }
  ...
  .tag.primary { color:var(--brand-primary); }
  .tag.light-color { color:var(--brand-light); }
  .tag.muted { color:rgba(255,255,255,0.5); }
  h1,h2 { font-family:'Playfair Display',serif; letter-spacing:-0.3px; line-height:1.1; }
  h1 { font-size:30px; font-weight:700; }
  h2 { font-size:28px; font-weight:600; }
  .body { font-size:14px; line-height:1.55; }
  .small { font-size:12px; line-height:1.5; }
  .logo-lockup { display:flex; align-items:center; gap:10px; margin-bottom:20px; }
  .logo-circle { width:40px; height:40px; border-radius:50%; background:var(--brand-primary); display:flex; align-items:center; justify-content:center; font-family:'Playfair Display',serif; color:#fff; font-size:16px; font-weight:700; flex-shrink:0; position:relative; overflow:hidden; }
  ...
  .step-row { display:flex; align-items:flex-start; gap:16px; padding:13px 0; border-bottom:1px solid rgba(255,255,255,0.08); }
  ...
  .step-title { font-size:14px; font-weight:600; color:#fff; display:block; }
  .step-desc { font-size:12px; color:rgba(255,255,255,0.5); display:block; margin-top:2px; }
  ...
  .ig-dot { width:6px; height:6px; border-radius:50%; background:#dbdbdb; transition:background 0.2s,width 0.2s; }
...
<div class="ig-frame">
  <div class="ig-header">
    ...
    <div>
      <div class="ig-handle">davimoxoto</div>
      <div class="ig-subtitle">IA aplicada a negócios</div>
    </div>
...
function aplicarFoto(dataUrl) {
  document.querySelectorAll('.avatar-photo').forEach(el => {
...
function carregarFoto(input) {
  ...
  reader.onload = function(e) {
    ...
    aplicarFoto(dataUrl);
    try { localStorage.setItem('davimoxoto_avatar', dataUrl); } catch(e) {}
    document.getElementById('fotoStatus').textContent = '✅ Foto salva!';
    document.getElementById('fotoStatus').style.color = '#2a6b1a';
  };
  reader.readAsDataURL(input.files[0]);
}
(function() {
  try {
    const saved = localStorage.getItem('davimoxoto_avatar');
    if (saved) {
      window.addEventListener('DOMContentLoaded', () => {
        aplicarFoto(saved);
        document.getElementById('fotoStatus').textContent = '✅ Foto carregada automaticamente.';
        document.getElementById('fotoStatus').style.color = '#2a6b1a';
      });
    }
  } catch(e) {}
...
async function exportarSlides() {
  ...
  if (typeof html2canvas === 'undefined') {
    status.textContent = '❌ html2canvas não carregou. Verifique sua conexão.';
    btn.disabled = false; btn.textContent = '⬇ Exportar novamente'; return;
  }
  const slides = document.querySelectorAll('.slide');
  const nomes = [${slideNames}];
  ...
  for (let i = 0; i < slides.length; i++) {
    status.textContent = '⏳ Capturando slide ' + (i+1) + ' de ' + slides.length + '...';
    try {
      const canvas = await html2canvas(slides[i], { scale:SCALE, useCORS:true, allowTaint:true, backgroundColor:null, width:420, height:525, logging:false, imageTimeout:10000 });
      let dataUrl;
      try { dataUrl = canvas.toDataURL('image/png'); }
      catch(e) {
        const canvas2 = await html2canvas(slides[i], { scale:SCALE, useCORS:false, allowTaint:false, backgroundColor:'#180E0C', width:420, height:525, logging:false, ignoreElements:el=>el.tagName==='IMG' });
        dataUrl = canvas2.toDataURL('image/png');
      }
      const link = document.createElement('a');
      link.download = (nomes[i] || 'slide_' + (i+1)) + '.png';
      link.href = dataUrl;
      ...
      status.textContent = '❌ Erro no slide ' + (i+1) + ': ' + e.message;
      await new Promise(r => setTimeout(r, 800));
    }
  }
  viewport.style.overflow = origOverflow;
  status.innerHTML = '✅ ' + ok + ' de ' + slides.length + ' slides exportados.';
  status.style.color = '#2a6b1a';
  btn.textContent = '⬇ Exportar novamente';
  btn.disabled = false;
}

const track = document.getElementById('track');
const viewport = document.getElementById('viewport');
const allDots = document.querySelectorAll('.ig-dot');
const TOTAL = ${total};
const SLIDE_W = 420;
...
export async function GET(request: NextRequest) {
  ...
  if (!piece) return NextResponse.json({ error: "Post não encontrado" }, { status: 404 });

  const slides = (Array.isArray(piece.slides) ? piece.slides : []) as Slide[];
  if (slides.length === 0) return NextResponse.json({ error: "Sem slides para exportar" }, { status: 400 });

  const html = generateHtml(slides, piece.title ?? "Carrossel");
  const filename = `carrossel_${piece.title?.toLowerCase().replace(/\s+/g, "_").slice(0, 30) ?? "post"}.html`;
