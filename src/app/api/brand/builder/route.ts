...
- Cubra ao longo da conversa: descrição da marca, público-alvo, pilares de conteúdo, frases características, palavras proibidas, inimigo/vilão, opiniões fortes, histórias/provas, dores e desejos do público, ofertas/CTAs, referências de estilo.
...
Use "done": true só quando o cérebro estiver bem completo, com uma mensagem de encerramento.`;
...
${docText ? `\nDOCUMENTOS DA MARCA (use para pré-preencher e sugerir):\n${docText}` : ""}`;
  ...
  if (extrasChanged) {
    await supabase
      .from("brands")
      .update({ identity: { ...(identity as object), brain_extras: newExtras } })
      .eq("id", body.brandId);
